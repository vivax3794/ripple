    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        ...
        if language_id == "css" {
            ...
            match ripple_parser::Document::parse(text, parser) {
                Ok(document) => {
                    let (processor, channel) =
                        DocumentProcessor::<css::Css>::new(uri.clone(), document);
...
trait Language: Default {
    fn diagnostics(&self, document: &mut ripple_parser::Document) -> Vec<Diagnostic>;
}
    ...
    async fn do_loop(mut self, client: Arc<Client>) {
        ...
        loop {
            client
                .publish_diagnostics(
                    self.url.clone(),
                    self.lang.diagnostics(&mut self.document),
                    None,
                )
                .await;
...
fn node_range(node: &ripple_parser::tree_sitter::Node) -> Range {
